...
void	render_column(t_data *d, int x)
{
	...
	// color_above(d, wall_height, ray_hit_wall_x, ray_hit_wall_y);
	// color_below(d, ray_hit_wall_x, ray_hit_wall_y);
	// if (d->ray->hit == 1)
	// {
	// 	double	texture_segment = d->elem->wallx * 100 * 0.63;
	// 	img_dis_col(d, &d->NESW[d->elem->texnum], wall_height, ray_hit_wall_x, ray_hit_wall_y, (int) (texture_segment));
	// }
